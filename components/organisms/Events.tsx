        ...
        <div className="flex flex-col md:flex-row items-center border rounded-lg overflow-hidden">
            <div className="flex flex-col gap-2 p-4 lg:p-6">
                ...
                <h2 className="text-gray-800 text-xl font-bold">
                    <Link href={event.event_url} target="_blank" className="hover:text-primary-500 active:text-primary-600 transition duration-100">{event.title}</Link>
                </h2>

                <p className="text-gray-500">{event.catch}</p>

                <div>
                    <Link href={event.event_url} target="_blank" className="text-primary-500 hover:text-primary-600 active:text-primary-700 font-semibold transition duration-100">詳しく見る</Link>
                </div>
            </div>
        </div>
    )
